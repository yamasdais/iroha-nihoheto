...
struct bytes_to_hexstr_fn {
    ...
    std::string operator()(IStart begin, IEnd end,
        ...
        if (uppercase)
            oss << std::uppercase;  // = oss.setf(std::ios_base::uppercase)

        std::ranges::for_each(begin, end, [&oss](auto v) {
...
template <std::ranges::forward_range Range>
void print_range(Range&& range, char const* prefix,
                 bool const uppercase = false) {
    std::cout << prefix
              << bytes_to_hexstr(std::forward<Range>(range), uppercase)
              << std::endl;
}
